...
func _update_task_list():
	...
	for i in TaskManagerSingleton.current_player_tasks:
		for prop_index in range(
			TaskManagerSingleton.current_player_tasks[i].minigame_scene.get_state().get_node_property_count(0)
		):
			if (
				TaskManagerSingleton.current_player_tasks[i].minigame_scene.get_state().get_node_property_name(
					0, prop_index
				)
				== "polish_name"
			):
				tasks_string += (
					...
					. current_player_tasks[i]
					. minigame_scene
					. get_state()
					. get_node_property_value(0, prop_index)
				)
				break
...
func _update_progress_bar():
	_progress_bar.value = int(
		float(TaskManagerSingleton.global_tasks_completed_amount) / TaskManagerSingleton.global_tasks_amount * 100
	)
