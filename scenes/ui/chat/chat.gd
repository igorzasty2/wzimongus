...
@onready var username = MultiplayerManager.get_current_player_info("username")
	...
	match group:
		Group.DEAD:
			if current_group == Group.DEAD:
				_create_message(MultiplayerManager.get_registered_player_info(id, "username"), message, Group.DEAD)
		Group.IMPOSTOR:
			if current_group == Group.IMPOSTOR:
				_create_message(MultiplayerManager.get_registered_player_info(id, "username"), message, Group.IMPOSTOR)
			else:
				_create_message(MultiplayerManager.get_registered_player_info(id, "username"), message, Group.GLOBAL)
		Group.SYSTEM:
		...
		_:
			_create_message(MultiplayerManager.get_registered_player_info(id, "username"), message, current_group)
	
	if multiplayer.is_server():
		for peer_id in MultiplayerManager.get_registered_players().keys():
			if peer_id != 1:
